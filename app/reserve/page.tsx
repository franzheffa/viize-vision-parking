  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-black text-slate-900 mb-8 italic tracking-tighter">RÉSERVER UNE PLACE</h1>
            ...
            <form key={spot.id} action={async () => {
              ...
              <button
                ...
                className={`w-full p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-2 ${
                  ...
                  ? 'border-white bg-white shadow-sm hover:border-indigo-500' 
                  : 'border-slate-200 bg-slate-100 opacity-50 cursor-not-allowed'
                }`}
              >
                <span className="text-xs font-black text-slate-400">PLACE</span>
                <span className="text-2xl font-black text-slate-900">{spot.id.slice(-2)}</span>
